...
function createSplash() {
    // Add the splash
    g = new THREE.CubeGeometry(10, 3, 2);
    // m = new THREE.MeshPhongMaterial({ map: ironTexture });
    // m = new THREE.MeshPhongMaterial({ map: brickTexture });
    m = new THREE.MeshBasicMaterial({ color: 0x00000 });
    splashMeshu = new THREE.Mesh(g, m);
    splashMeshu.position.set(camera.position.x, 1, 2);
    scene.add(splashMeshu);

    splashMeshd = new THREE.Mesh(g, m);
    splashMeshd.position.set(camera.position.x, 1, 2);
    scene.add(splashMeshd);
}

function updateSplash() {
    splashMeshu.position.set(camera.position.x, endingClock.getElapsedTime() * 2 + 1, 2);
    splashMeshd.position.set(camera.position.x, -endingClock.getElapsedTime() * 2 + 1, 2);
}
...
function gameLoop() {
    ...
    switch (gameState) {
        ...
        case 'play':
            // 화면전환
            updateSplash();
            if ((endingClock.getElapsedTime() >= 3) && (created)) {
                scene.remove(splashMeshu);
                scene.remove(splashMeshd);
                created = false;
            }
            ...
            if (!appearance) {
                ...
                if (eMesh.scale.x <= 7) {
                    ...
                    if (visit == 0) {
                        visit++;
                        music.pause();
                        document.location.href = 'http://192.249.18.155:443/#/mazeaster';
                    } else {
                        scene.remove(eMesh);
                        gameState = "play";
                    }
        ...
        case 'end':
            ...
            if (Math.abs(light.intensity - 0.0) < 0.1) {
                ...
                document.location.href = 'http://192.249.18.155:443/ending';
            }
            break;
